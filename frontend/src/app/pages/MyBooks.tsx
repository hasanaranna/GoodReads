...
export function MyBooks() {
  ...
  const filtered = books.filter((b) => {
    ...
    const matchesSearch =
      ...
      (b.titleLocal && b.titleLocal.includes(searchQuery)) ||
      b.author.toLowerCase().includes(searchQuery.toLowerCase());
    return matchesShelf && matchesSearch;
  });

  const sorted = [...filtered].sort((a, b) => {
    if (sortBy === 'title') return (a.titleLocal || a.title).localeCompare(b.titleLocal || b.title);
  ...
  return (
    <div className="max-w-[1100px] mx-auto px-4 py-5">
      ...
      <div className="text-[13px] text-[#382110] mb-2">
        <Link to="/mybooks" className="no-underline text-[#382110] hover:underline">
          My Books
        </Link>
        {shelfLabel && (
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        ...
        <div className="ml-auto flex items-center gap-3 text-[13px]">
          ...
          {batchMode && (
            <>
              <button onClick={selectAll} className="text-[#00635d] hover:underline text-[12px]">
                Select All
              </button>
              <button
          ...
          <button className="font-semibold text-[#382110] hover:underline">Settings</button>
          <button className="font-semibold text-[#382110] hover:underline">Stats</button>
          <button className="font-semibold text-[#382110] hover:underline">Print</button>
          <span className="text-[#ccc]">|</span>
          <button
            onClick={() => setViewMode('list')}
            className={viewMode === 'list' ? 'text-[#382110]' : 'text-[#aaa] hover:text-[#382110]'}
          >
            <List size={18} />
          </button>
          <button
            onClick={() => setViewMode('grid')}
            className={viewMode === 'grid' ? 'text-[#382110]' : 'text-[#aaa] hover:text-[#382110]'}
          >
            <LayoutGrid size={18} />
          </button>
        </div>
              ...
              {(['title', 'author', 'rating', 'dateAdded'] as const).map((s) => (
                <button
                  ...
                  className={`capitalize hover:underline ${
                    sortBy === s ? 'text-[#382110] underline' : 'text-[#00635d]'
                  }`}
                >
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                <button
                  ...
                  className={`px-1 hover:underline ${
                    currentPage === p ? 'text-[#382110] underline' : 'text-[#00635d]'
                  }`}
                >
                  {p}
                </button>
          ...
          {viewMode === 'list' ? (
            <div>
              ...
              <div className="flex items-center gap-3 py-2 border-b-2 border-[#382110] text-[12px] text-gray-500">
                ...
                <div className="hidden md:block w-[110px] shrink-0">Date Added / Read</div>
            ...
            <div className="flex flex-wrap gap-4 pt-2">
              {paginated.length === 0 ? (
                <div className="py-12 text-center text-gray-400 text-[14px] w-full">No books found.</div>
              ) : (
                paginated.map((book) => (
                  <BookRow
                    key={book.id}
